...
- **远期开放项（不进 V1 预算）**：把后端搬到 VPS 当瘦客户端是另一个产品形态——Pig 的承重数据（`~/.pi/agent` 日志、Execution Checkout 的 Git worktree、Pi 进程）都在用户本机，真要远程需把整套文件/checkout 语义搬到服务端，并支付跨网络信任边界的固定税（auth/TLS/重连）。"后端可重定位"这个抽象能力 ≠ "VPS 部署"这个产品。
