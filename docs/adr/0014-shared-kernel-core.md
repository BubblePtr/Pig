# 领域契约收进 shared-kernel core（`@pig/core`）

Pig 的 ubiquitous-language 契约类型此前散落在 UI 模块里（`session-detail.tsx`、`setup.tsx`、`sessions.ts`、`pi-runtime-bridge.ts`），且 utilityProcess 后端（`src/backend/*`）跨进程边界**反向 import 渲染层 UI 文件**去取这些类型。本决策把"跨 seam 共享的契约"抽进一个独立 module `@pig/core`，渲染层与后端都依赖它、互不依赖。
...
core 现在是 `src/core/`（带 `index.ts` barrel 作为唯一 public 面），通过 alias `@pig/core` 被消费，不是一个 workspace 包。
...
- 升级路径机械无痛：`git mv src/core packages/core` + 加 package.json，因为 import 边界与 barrel 此时已就位；alias 已用未来包名 `@pig/core`，连改名都省。

## Consequences

- **import 单向**：渲染层 → core、后端 → core，**两侧互不 import**。后端不再 import 任何 `src/*.tsx`。
- barrel `src/core/index.ts` 是 core 的唯一 public interface；外部不深 import `@pig/core/...` 内部文件。
- alias 配三处保持一致：`tsconfig.json`（tsc/IDE）、`electron.vite.config.ts`（app 构建）、`vite.config.ts`（vitest）。
- 旧定义文件改为从 `@pig/core` import 并对存量 UI 消费者再导出；契约的权威定义只在 core。
